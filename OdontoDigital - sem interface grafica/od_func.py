...
#Funções dos Gerenciadores
def cadastrarGerenciadores():
    '''Realiza o cadastro do gerenciador no banco de dados.'''
    nome = input("Nome: ")
    login = input("Login: ")
    senha = input("Senha: ")
    listarClinicas()
    idclinica = int(input("Digite o ID da clínica vinculada: "))
    pesquisarGerenciadores(idclinica)
    con = sqlite3.connect("odonto.db")
    cursor = con.cursor()
    cursor.execute('SELECT nome FROM clinicas WHERE id=?', [idclinica])
    c= cursor.fetchone()
    clinica = c[0]
    cursor.execute("INSERT INTO gerenciadores (nome, login, senha, clinica) VALUES (?,?,?,?)", [nome, login, senha, clinica])
    con.commit()
    con.close()

def listarGerenciadores():
    '''Lista todos os gerenciadores cadastrados.'''
    con = sqlite3.connect("odonto.db")
    cursor = con.cursor()
    cursor.execute("SELECT * FROM gerenciadores;")
    gerenciador = cursor.fetchall()
    con.commit()
    con.close()
    print('\nLISTA DE GERENCIADORES:\n')
    for i in gerenciador:
        print(f'ID: {i[0]} | Nome: {i[1]} | Clínica: {i[4]} ')
    print()

def pesquisarGerenciadores(id):
    '''Retorna somente o registro especificado pelo ID.'''
    con = sqlite3.connect("odonto.db")
    cursor = con.cursor()
    cursor.execute("SELECT * FROM gerenciadores  WHERE ID=?;", [id])
    gerenciador = cursor.fetchall()
    for i in gerenciador:
        print(f'ID: {i[0]} | Nome: {i[1]} | Clínica: {i[4]}')
    con.commit()
...
def atualizarGerenciadores():
    '''Atualiza um registro específico no Banco.'''
    listarGerenciadores()
    id = int(input('\nInsira o ID do registro que deseja atualizar: '))
    pesquisarGerenciadores(id)
    print('---------------------------------------------------------------------------------------------------------------------------------')
    nome = input("Nome: ")
    login = input("Login: ")
    senha = input("Senha: ")
    ...
    c= cursor.fetchone()
    clinica = c[0]
    cursor.execute("UPDATE gerenciadores SET nome=?,clinica=?,login=?,senha=? WHERE ID=?", 
    [nome, clinica, login, senha, id])
    con.commit()
    con.close()
    print()
    listarGerenciadores()

def excluirGerenciadores():
    '''Deleta um registro específico no banco.'''
    listarGerenciadores()
    id = int(input('\nInsira o ID do registro que deseja excluir: '))
    pesquisarGerenciadores(id)
    ...
    if r == 1:
        con = sqlite3.connect("odonto.db")
        cursor = con.cursor()
        cursor.execute("DELETE FROM gerenciadores WHERE ID=?", [id])
        con.commit()
        con.close()
        print('Registro deletado com sucesso!')
        listarGerenciadores()
